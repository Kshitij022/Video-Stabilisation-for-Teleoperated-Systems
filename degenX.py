...
###################################################################################################################################################################
		...
		self.smoothedTransforms.append([dx, dy, da])
...
###############################################################################################################################################################
	...
	def showGraph(self):
		...
		self.smoothedTransforms = np.array(self.smoothedTransforms)
		diff = self.smoothedTransforms - self.transforms
		trajectory_smooth = trajectory + diff
		dx_traj = trajectory[:, 0]
		dy_traj = trajectory[:, 1]
		da_traj = trajectory[:, 2]
		smoothed_dx_traj = trajectory_smooth[:, 0]
		smoothed_dy_traj = trajectory_smooth[:, 1]
		smoothed_da_traj = trajectory_smooth[:, 2]

		plt.subplot(3, 1, 1)
		plt.plot(np.arange(len(dx_traj)), dx_traj, color='r', label='dx')
		plt.plot(np.arange(len(smoothed_dx_traj)), smoothed_dx_traj, color='g', label='smoothed dx')
		plt.legend()
		plt.subplot(3, 1, 2)
		plt.plot(np.arange(len(dy_traj)), dy_traj, color='r', label='dy')
		plt.plot(np.arange(len(smoothed_dy_traj)), smoothed_dy_traj, color='g', label='smoothed dy')
		plt.legend()
		plt.subplot(3, 1, 3)
		plt.plot(np.arange(len(da_traj)), da_traj, color='r', label='da')
		plt.plot(np.arange(len(smoothed_da_traj)), smoothed_da_traj, color='g', label='smoothed da')
		plt.legend()
		plt.show()

		# plt.subplot(2,1,1)
		# plt.plot(np.arange(len(self.K_collect)), self.K_collect, color='r')
		# plt.title("dx")
		# plt.subplot(2,1,2)
		# plt.plot(np.arange(len(self.P_collect)), self.P_collect, color='b')
		# plt.title("dy")
		plt.show()
